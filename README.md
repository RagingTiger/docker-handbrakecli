...
to run it you will need to run the image as such:
```
$ docker run --name hbcli \
            -it tigerj/hbcli \
            HandBrakeCLI -i /path/to/input -o /path/to/output
```

## Shell Access
To login to a running container, use either the container `name` (i.e.
`--name hbcli` from *usage* example) or id (i.e. check with `docker ps -a`) and
then execute:
```
$ docker exec -it $CONTAINER_NAME_OR_ID bash
```
From here you can poke around in the file system

## Copying Files
Using `docker cp` it is possible to copy files out of the container:
```
$ docker cp $CONTAINER_NAME_OR_ID:/path/to/your/file /path/to/host/directory
```

## Storage / Volumes
Using the `-v` option we can mount a directory from `host` to `container` as
follows:
```
$ docker run -d \
             --name hbcli \
             -v /host/input/path:/container/input/path
             -v /host/output/path:/host/output/path
             tigerj/hbcli \
             HandBrakeCLI -i /container/input/path/YOURFILE.mkv \
                          -o /container/output/path/YOURFILE.m4v
```
...
Subtitles Options ------------------------------------------------------------
  ...
  -N, --native-language <string>
                           ...
                           then select the first subtitleHandBrake has exited.
 that does. When
